...
## 2. Karax의 역할 — 래퍼이자 오케스트레이터

Karax는 외부 도구를 직접 대체하지 않는다. **래퍼로 감싸서** 일관된 인터페이스를 제공하고, 결과를 Karax의 분석 체계에 통합한다.

```
외부 도구                     Karax 래퍼                    Karax 출력
─────────                   ──────────                   ──────────
...
| 규칙 ID | 심각도 | 설명 | 규정 근거 |
|---------|--------|------|-----------|
| `karax/field-type-required` | error | 모든 필드에 type 필수 | § 2.2 |
| `karax/nullable-explicit` | error | nullable 필드는 `nullable: true` 명시 | § 2.3.1 |
| `karax/no-nullable-optional` | warn | nullable + optional 동시 적용 금지 | § 2.3.1 |
| `karax/error-response-schema` | error | 4xx/5xx → ErrorResponse 스키마 참조 | § 2.3.2 |
| `karax/pagination-required` | warn | 배열 응답에 페이지네이션 필수 | § 2.3.3 |
| `karax/path-naming` | error | 엔드포인트 kebab-case | § 3.1 |
| `karax/property-naming` | error | 필드명 camelCase | § 3.1 |
| `karax/enum-required` | warn | 값이 유한 집합이면 enum 사용 | § 2.4.1 |
| `karax/example-required` | warn | 날짜/금액/ID에 example 필수 | § 2.4.2 |
| `karax/deprecated-lifecycle` | error | deprecated 표시 없이 삭제 금지 | § 2.4.3 |
...
Karax는 자체 경량 린트 엔진을 내장한다. OpenAPI JSON을 직접 파싱하여 위 10개 규칙을 검사한다. Spectral이 있으면 Spectral을 사용하고, 없으면 내장 엔진으로 폴백한다.

```
Spectral 설치됨?
  ├─ Yes → Spectral 실행 (karax 내장 .spectral.yaml 주입) → 결과를 ApiLintResult로 변환
...
Karax는 자체 경량 diff 엔진을 내장한다. 두 OpenAPI JSON을 비교하여 엔드포인트/필드 수준의 변경을 탐지한다. oasdiff가 있으면 더 정밀한 분석을 제공하고, 없으면 내장 엔진으로 기본 탐지를 수행한다.
...
기존 `karax check`에 v0.2 기능을 통합하고, 새 서브커맨드를 추가한다.
...
`karax check --json` 결과를 기반으로 구조화된 코드 리뷰를 수행한다.

```
입력:
  - karax check --json 결과 (범위 분석 + 리뷰 체크리스트)
  - karax api:lint --json 결과 (API 린트, 해당 시)
  - karax api:diff --json 결과 (API diff, 해당 시)
...
| **CLI: `karax api:lint`** | API 스펙 린트 실행 | CLI |
| **CLI: `karax api:diff`** | API 스펙 diff 실행 | CLI |
| **CLI: `karax review`** | 리뷰 체크리스트 생성 | CLI |
| **CLI: `karax check` 확장** | 범위 분석 + 리뷰 체크리스트 통합 출력 | CLI |
| **code-reviewer 에이전트** | karax 결과 기반 구조화 리뷰 | 에이전트 |
