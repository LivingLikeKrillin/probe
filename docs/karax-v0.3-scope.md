...
> **핵심 가치**: Claude Code에서 Karax의 모든 분석 기능을 자연어로 호출하고, 결과를 실시간으로 받아본다. MCP(Model Context Protocol) 서버로 Karax를 Claude Code의 도구로 등록한다.
...
**Karax의 분석 엔진을 MCP 서버로 노출하여, Claude Code가 자연어 대화 중에 PR 범위 분석/API 린트/리뷰 체크리스트를 도구로 호출할 수 있게 한다.**
...
v0.1~v0.2의 Karax는 두 가지 방식으로 동작한다:
1. **CLI** — `npx karax check`, `npx karax api:lint` 등
2. **Claude Code skills/hooks** — `.claude/skills/`에서 `npx karax` 를 쉘로 호출

두 방식 모두 **프로세스를 fork하고 텍스트 출력을 파싱**한다. 이 구조의 문제:

```
현재: Claude Code → shell → npx karax → stdout 텍스트 → Claude Code가 텍스트 파싱
      (비효율: 프로세스 fork, 텍스트 직렬화/역직렬화, 에러 핸들링 복잡)

목표: Claude Code → MCP tool call → Karax 분석 엔진 → 구조화된 JSON 응답
...
```
사용자: "PR 만들어줘"
Claude: /check-scope 실행 → 쉘에서 npx karax check → 텍스트 결과 읽기 → 해석 → 응답
```

**After (v0.3):**
```
사용자: "PR 만들어줘"
Claude: (자동으로 karax.analyzeScope 도구 호출) → 구조화된 결과 → 즉시 판단 → 응답
        (자동으로 karax.reviewChecklist 도구 호출) → 체크리스트 → 리뷰 포인트 포함
```

사용자가 명시적으로 커맨드를 입력하지 않아도, Claude가 맥락에 따라 Karax 도구를 **자동으로** 호출한다.
...
Karax MCP 서버는 **stdio transport**를 사용한다. Claude Code의 MCP 설정에 등록하면 상주 프로세스로 동작한다.
...
### 3.1 `karax.analyzeScope`

PR 범위를 분석한다. v0.1 코어 엔진 직접 호출.

```typescript
{
  name: "karax.analyzeScope",
...
### 3.2 `karax.lintApiSpec`

OpenAPI 스펙을 린트한다. v0.2 내장 린트 엔진 직접 호출.

```typescript
{
  name: "karax.lintApiSpec",
...
### 3.3 `karax.diffApiSpecs`

두 버전의 API 스펙을 비교한다.

```typescript
{
  name: "karax.diffApiSpecs",
...
### 3.4 `karax.reviewChecklist`

리뷰 체크리스트를 생성한다.

```typescript
{
  name: "karax.reviewChecklist",
...
### 3.5 `karax.detectPlatform`

프로젝트의 플랫폼을 감지한다.

```typescript
{
  name: "karax.detectPlatform",
...
### 4.1 `karax://profiles/{platform}`

플랫폼 프로파일 정보를 제공한다.

```
karax://profiles/spring-boot  → Spring Boot 프로파일 JSON
karax://profiles/nextjs        → Next.js 프로파일 JSON
karax://profiles/react-spa     → React SPA 프로파일 JSON
```

### 4.2 `karax://config`

현재 프로젝트의 karax 설정을 제공한다.

```
karax://config  → 로드된 KaraxConfig JSON
```

### 4.3 `karax://guidelines/{name}`

규정 문서를 텍스트로 제공한다.

```
karax://guidelines/api-contract      → 규정 ② API 계약 가이드라인
karax://guidelines/state-matrix      → 규정 ① State Matrix 가이드라인
karax://guidelines/ui-change         → 규정 ③ UI 변경 가이드라인
```
...
{
  name: "karax.prReview",
  description: "현재 변경에 대해 karax 분석 결과를 기반으로 구조화된 코드 리뷰를 수행한다.",
  arguments: [
...
프롬프트 생성 시 `karax.analyzeScope`와 `karax.reviewChecklist` 결과를 포함한 리뷰 지시 프롬프트를 반환한다.
...
| 기존 | MCP 대응 | 공존 |
|------|----------|------|
| `/check-scope` skill | `karax.analyzeScope` 도구 | skill은 MCP 미지원 환경 폴백 |
| `/split-pr` skill | `karax.splitPr` 프롬프트 | 동일 |
| `/state-matrix` skill | (없음, v0.4 범위) | skill 유지 |
| PostToolUse hook | (유지) | hook은 자동 실행, MCP는 요청 시 |
| code-reviewer agent | `karax.prReview` 프롬프트 | agent가 MCP 도구를 호출하도록 개선 |
| test-writer agent | (유지) | agent 자체는 MCP 불필요 |

### Skills → MCP 마이그레이션

기존 skills는 내부적으로 `npx karax` 쉘 호출을 하지만, MCP가 활성화된 환경에서는 Claude가 MCP 도구를 우선 사용한다. Skills는 MCP가 없는 환경의 폴백으로 유지.
...
{
  "bin": {
    "karax": "dist/cli/index.js",
    "karax-mcp": "dist/mcp/server.js"
  }
}
```
...
Claude: (내부적으로 karax.analyzeScope 호출)
        (내부적으로 karax.reviewChecklist 호출)
        (내부적으로 karax.lintApiSpec 호출 — API 스펙 변경 감지 시)
...
Claude: (PR 생성 전에 karax.analyzeScope 자동 호출)
...
Claude: (karax://guidelines/api-contract 리소스 참조)
...
| MCP sampling | Claude ↔ Karax 양방향 호출은 과도 | v0.4+ |
